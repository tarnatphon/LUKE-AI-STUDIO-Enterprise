#!/usr/bin/env node
"use strict";

const fs = require("node:fs");
const projects = fs.readFileSync("app/frontend/src/components/ChatProjects.jsx", "utf8");
const app = fs.readFileSync("app/frontend/src/App.jsx", "utf8");
const sidebar = fs.readFileSync("app/frontend/src/components/Sidebar.jsx", "utf8");
const chat = fs.readFileSync("app/frontend/src/components/TextChat.jsx", "utf8");
const styles = fs.readFileSync("app/frontend/src/App.css", "utf8");
const workTools = fs.readFileSync("app/frontend/src/components/WorkToolsPanel.jsx", "utf8");
const server = fs.readFileSync("scripts/server/serve.cjs", "utf8");
const inspector = fs.readFileSync("scripts/server/work-environment-inspector.cjs", "utf8");
const actionRunner = fs.readFileSync("scripts/server/work-action-runner.cjs", "utf8");
const terminalDock = fs.readFileSync("app/frontend/src/components/WorkTerminalDock.jsx", "utf8");
const projectMemory = fs.readFileSync("app/frontend/src/components/ProjectMemoryPanel.jsx", "utf8");
const workFiles = fs.readFileSync("scripts/server/work-file-manager.cjs", "utf8");
const folderGrants = fs.readFileSync("scripts/server/work-folder-grants.cjs", "utf8");
const projectSearch = fs.readFileSync("scripts/server/work-project-search.cjs", "utf8");

function requireText(source, text, label) {
  if (!source.includes(text)) throw new Error(`${label}: ${text}`);
}

requireText(app, 'localStorage.getItem("chat_projects")', "PROJECT_PERSISTENCE_MISSING");
requireText(app, "conversationHistoryHydrated", "CHAT_HISTORY_HYDRATION_GUARD_MISSING");
requireText(app, 'localStorage.setItem("chat_conversations"', "CHAT_HISTORY_LOCAL_MIRROR_MISSING");
requireText(app, "conversationSaveChainsRef", "CHAT_HISTORY_SERIAL_SAVE_MISSING");
requireText(app, "projectId: activeProjectId", "CONVERSATION_PROJECT_LINK_MISSING");
requireText(sidebar, "<ChatProjects", "PROJECT_SIDEBAR_MISSING");
requireText(projects, 'aria-label="Create project"', "CREATE_PROJECT_MISSING");
requireText(projects, 'role="dialog"', "PROJECT_EDITOR_DIALOG_MISSING");
requireText(projects, "sourceFolders", "SOURCE_FOLDER_REFERENCES_MISSING");
requireText(projects, "folderGrants", "SOURCE_FOLDER_GRANTS_MISSING");
requireText(projects, 'purpose: "work-project-source"', "SOURCE_FOLDER_GRANT_PICKER_MISSING");
requireText(projects, 'fetch("/api/storage/choose-folder"', "NATIVE_FOLDER_PICKER_MISSING");
requireText(projects, "!draftFolders.includes(selectedPath)", "DUPLICATE_FOLDER_GUARD_MISSING");
requireText(projects, "Source folders and files will not be deleted", "NON_DESTRUCTIVE_REMOVAL_NOTICE_MISSING");
requireText(projects, "conversation.projectId === project.id", "PROJECT_CHAT_FILTER_MISSING");
requireText(projects, "pinned", "PROJECT_PINNING_MISSING");
requireText(app, 'localStorage.getItem("luke_assistant_mode")', "ASSISTANT_MODE_PERSISTENCE_MISSING");
requireText(app, "if (nextMode === assistantMode)", "ASSISTANT_MODE_IDEMPOTENCE_MISSING");
requireText(app, "conversationMode(conversation) === nextMode", "MODE_CONVERSATION_RESTORE_MISSING");
if (/setAssistantMode=\{\(mode\)[\s\S]{0,180}setActiveConversationId\(null\)/.test(app)) {
  throw new Error("MODE_SWITCH_MUST_NOT_ALWAYS_CLEAR_CONVERSATION");
}
requireText(app, 'className="workspace-panel"', "STABLE_WORKSPACE_PANEL_MISSING");
requireText(styles, ".workspace-panel", "STABLE_WORKSPACE_LAYOUT_MISSING");
requireText(styles, "min-height: 0", "WORKSPACE_MIN_HEIGHT_GUARD_MISSING");
requireText(sidebar, 'setAssistantMode?.("chat")', "CHAT_MODE_OPTION_MISSING");
requireText(sidebar, 'setAssistantMode?.("work")', "WORK_MODE_OPTION_MISSING");
requireText(sidebar, 'aria-label="LUKE AI Studio home"', "SIDEBAR_BRAND_MISSING");
requireText(sidebar, 'role="tablist" aria-label="Choose Chat or Work mode"', "VISIBLE_MODE_TABS_MISSING");
requireText(sidebar, 'aria-selected={assistantMode === "chat"}', "CHAT_MODE_SELECTION_STATE_MISSING");
requireText(sidebar, 'aria-selected={assistantMode === "work"}', "WORK_MODE_SELECTION_STATE_MISSING");
requireText(sidebar, 'assistantMode === "work" && <ChatProjects', "WORK_PROJECT_VISIBILITY_MISSING");
requireText(sidebar, "onContextMenu", "CHAT_CONTEXT_MENU_MISSING");
requireText(sidebar, "Move to Work project", "MOVE_CHAT_TO_PROJECT_MISSING");
requireText(sidebar, "onMoveConversationToProject", "MOVE_CHAT_HANDLER_MISSING");
requireText(chat, 'assistantMode === "work"', "WORK_MODE_PROMPT_MISSING");
requireText(chat, 'if (assistantMode === "work") return;', "CHAT_MODE_WORK_PANEL_CLEANUP_MISSING");
requireText(chat, "Project source folders:", "WORK_SOURCE_FOLDER_CONTEXT_MISSING");
requireText(chat, 'label="Copy code"', "COPY_CODE_BUTTON_MISSING");
requireText(chat, 'label="Copy response"', "COPY_RESPONSE_BUTTON_MISSING");
requireText(chat, 'localStorage.getItem("luke_work_approval_mode")', "APPROVAL_MODE_PERSISTENCE_MISSING");
requireText(chat, '"Ask for approval"', "ASK_APPROVAL_MODE_MISSING");
requireText(chat, '"Approve for me"', "AUTO_APPROVAL_MODE_MISSING");
requireText(chat, '"Full Access"', "FULL_ACCESS_MODE_MISSING");
requireText(chat, '"Custom"', "CUSTOM_APPROVAL_MODE_MISSING");
requireText(chat, 'window.confirm("Full Access', "FULL_ACCESS_CONFIRMATION_MISSING");
requireText(chat, "<WorkToolsPanel", "WORK_TOOLS_PANEL_MISSING");
requireText(workTools, 'fetch("/api/work/environment"', "WORK_ENVIRONMENT_FETCH_MISSING");
requireText(workTools, 'useState("environment")', "ENVIRONMENT_TAB_MISSING");
requireText(workTools, 'tab === "review"', "REVIEW_TAB_MISSING");
requireText(workTools, 'fetch("/api/work/review/diff"', "WORK_REVIEW_DIFF_FETCH_MISSING");
requireText(workTools, "Staged + unstaged", "WORK_REVIEW_DIFF_SUMMARY_MISSING");
requireText(workTools, 'tab === "files"', "FILES_TAB_MISSING");
requireText(workTools, 'tab === "search"', "PROJECT_SEARCH_TAB_MISSING");
requireText(workTools, 'aria-label="Search all Project files"', "PROJECT_SEARCH_INPUT_MISSING");
requireText(workTools, 'fetch("/api/work/search"', "PROJECT_SEARCH_PANEL_API_MISSING");
requireText(workTools, "result.root", "PROJECT_SEARCH_MULTI_ROOT_OPEN_MISSING");
requireText(workTools, "searchIndexes", "PROJECT_SEARCH_INDEX_STATUS_MISSING");
requireText(workTools, "Reindex", "PROJECT_SEARCH_REINDEX_ACTION_MISSING");
requireText(server, "invalidateProjectIndex", "PROJECT_SEARCH_CACHE_INVALIDATION_MISSING");
requireText(server, 'req.url === "/api/work/environment"', "WORK_ENVIRONMENT_API_MISSING");
requireText(inspector, 'shell: false', "WORK_GIT_SHELL_GUARD_MISSING");
requireText(inspector, '["status", "--porcelain=v1", "--branch"]', "WORK_GIT_STATUS_MISSING");
requireText(workTools, 'tab === "terminal"', "WORK_TERMINAL_TAB_MISSING");
requireText(workTools, 'tab === "browser"', "WORK_BROWSER_TAB_MISSING");
requireText(workTools, 'approvalGranted: true', "WORK_OPEN_APPROVAL_MISSING");
requireText(server, 'req.url === "/api/work/command"', "WORK_COMMAND_API_MISSING");
requireText(server, 'req.url === "/api/work/review/diff"', "WORK_REVIEW_DIFF_API_MISSING");
requireText(actionRunner, "runWorkFileDiff", "WORK_REVIEW_DIFF_RUNNER_MISSING");
requireText(server, 'req.url === "/api/work/open"', "WORK_OPEN_API_MISSING");
requireText(actionRunner, "READ_ONLY_COMMANDS", "READ_ONLY_COMMAND_PALETTE_MISSING");
requireText(actionRunner, 'shell: false', "WORK_ACTION_SHELL_GUARD_MISSING");
requireText(actionRunner, "approvalGranted !== true", "WORK_OPEN_BACKEND_APPROVAL_MISSING");
requireText(chat, "<WorkTerminalDock", "BOTTOM_TERMINAL_DOCK_MISSING");
requireText(chat, 'title="Toggle bottom Terminal"', "BOTTOM_TERMINAL_TOGGLE_MISSING");
requireText(terminalDock, 'fetch("/api/work/terminal"', "BOTTOM_TERMINAL_API_MISSING");
requireText(terminalDock, 'fetch("/api/work/terminal/session"', "TERMINAL_FOLDER_SESSION_MISSING");
requireText(terminalDock, "terminalSession?.prompt", "TERMINAL_NATIVE_PROMPT_MISSING");
requireText(server, 'req.url === "/api/work/terminal/session"', "TERMINAL_SESSION_API_MISSING");
requireText(server, "projectId: conversation.projectId", "PROJECT_HISTORY_SERVER_METADATA_MISSING");
requireText(terminalDock, 'aria-label="Bottom Work Terminal"', "BOTTOM_TERMINAL_ACCESSIBILITY_MISSING");
requireText(terminalDock, 'aria-label="Work Terminal command"', "EDITABLE_TERMINAL_INPUT_MISSING");
requireText(terminalDock, 'event.key === "ArrowUp"', "TERMINAL_HISTORY_MISSING");
requireText(terminalDock, "commandQueue", "TERMINAL_COMMAND_QUEUE_MISSING");
requireText(terminalDock, 'aria-label="Queued Terminal commands"', "TERMINAL_COMMAND_QUEUE_UI_MISSING");
requireText(terminalDock, 'busy ? "Queue" : "Run"', "TERMINAL_NON_BLOCKING_SUBMIT_MISSING");
requireText(terminalDock, "Type the next command while this one runs", "TERMINAL_BUSY_COMPOSER_PROMPT_MISSING");
requireText(terminalDock, "terminalSessionKey", "TERMINAL_SESSION_STORAGE_KEY_MISSING");
requireText(terminalDock, "MAX_SAVED_DRAFT_CHARS", "TERMINAL_DRAFT_STORAGE_BOUND_MISSING");
requireText(terminalDock, "MAX_SAVED_HISTORY", "TERMINAL_HISTORY_STORAGE_BOUND_MISSING");
requireText(terminalDock, "localStorage.setItem(sessionKey", "TERMINAL_SESSION_PERSISTENCE_MISSING");
requireText(terminalDock, "window.setTimeout", "TERMINAL_PERSISTENCE_DEBOUNCE_MISSING");
if (/aria-label="Work Terminal command"[^>]*disabled=\{busy\}/.test(terminalDock)) {
  throw new Error("BUSY_TERMINAL_COMPOSER_MUST_REMAIN_EDITABLE");
}
requireText(server, 'req.url === "/api/work/terminal"', "TYPED_TERMINAL_API_MISSING");
requireText(actionRunner, "runTypedWorkCommand", "TYPED_TERMINAL_RUNNER_MISSING");
requireText(actionRunner, "Pipes, redirection, substitutions", "TERMINAL_METACHAR_GUARD_MISSING");
requireText(chat, "messageQueue", "MESSAGE_QUEUE_STATE_MISSING");
requireText(chat, 'aria-label="Queued messages"', "MESSAGE_QUEUE_UI_MISSING");
requireText(chat, "MAX_RECOVERED_QUEUE_ITEMS", "MESSAGE_QUEUE_RECOVERY_BOUND_MISSING");
requireText(chat, "MAX_RECOVERED_QUEUE_TEXT_CHARS", "MESSAGE_QUEUE_TEXT_BOUND_MISSING");
requireText(chat, "persistRecoverableMessageQueue", "MESSAGE_QUEUE_PERSISTENCE_MISSING");
requireText(chat, "Context refreshed automatically", "CONTEXT_OVERFLOW_RECOVERY_MISSING");
requireText(chat, "Earlier content clipped from active model context", "CONTEXT_MESSAGE_CLIPPING_MISSING");
requireText(chat, "!item.attachments?.length", "MESSAGE_QUEUE_ATTACHMENT_STORAGE_GUARD_MISSING");
requireText(chat, "messageQueuePaused", "RECOVERED_QUEUE_PAUSE_MISSING");
requireText(chat, 'aria-label="Resume recovered queued messages"', "RECOVERED_QUEUE_RESUME_MISSING");
requireText(chat, "memo(function MarkdownRenderer", "MARKDOWN_RENDER_MEMOIZATION_MISSING");
requireText(chat, "onSendToTerminal={sendCodeToTerminal}", "STABLE_TERMINAL_CALLBACK_MISSING");
requireText(chat, "Add message to queue", "MESSAGE_QUEUE_SEND_ACTION_MISSING");
requireText(chat, "Edit queued message", "MESSAGE_QUEUE_EDIT_MISSING");
requireText(chat, "Move queued message", "MESSAGE_QUEUE_REORDER_MISSING");
requireText(chat, 'disabled={!status.ready}', "BUSY_COMPOSER_MUST_REMAIN_EDITABLE");
requireText(chat, 'aria-label="Search chat messages"', "CHAT_SEARCH_INPUT_MISSING");
requireText(chat, "messageSearchResults", "PROJECT_CHAT_SEARCH_MISSING");
requireText(chat, 'aria-label="Next search result"', "CHAT_SEARCH_NAVIGATION_MISSING");
requireText(chat, "branchAlternatives", "CHAT_RESPONSE_BRANCH_STORAGE_MISSING");
requireText(chat, 'aria-label="Retry response"', "CHAT_RETRY_ACTION_MISSING");
requireText(chat, 'aria-label="Edit message and retry"', "CHAT_EDIT_RETRY_ACTION_MISSING");
requireText(chat, 'aria-label="Response branches"', "CHAT_BRANCH_SWITCHER_MISSING");
requireText(chat, "getProjectMemory", "PROJECT_MEMORY_PROMPT_MISSING");
requireText(chat, "createWorkCheckpoint", "AUTOMATIC_WORK_CHECKPOINT_MISSING");
requireText(projectMemory, 'aria-label="Project Memory and checkpoints"', "PROJECT_MEMORY_PANEL_MISSING");
requireText(projectMemory, "Conversation checkpoints", "WORK_CHECKPOINT_UI_MISSING");
requireText(projectMemory, "Project files are never changed", "CHECKPOINT_SCOPE_WARNING_MISSING");
requireText(chat, 'aria-label="Composer commands and mentions"', "COMPOSER_COMMAND_MENU_MISSING");
requireText(chat, '"/checkpoint"', "CHECKPOINT_SLASH_COMMAND_MISSING");
requireText(chat, '"@file"', "FILE_MENTION_MISSING");
requireText(chat, 'readXlsxFile from "read-excel-file/browser"', "XLSX_CHAT_ATTACHMENT_MISSING");
requireText(chat, "addAttachmentFiles", "MULTI_FILE_ATTACHMENT_PIPELINE_MISSING");
requireText(chat, "onDrop=", "CHAT_FILE_DROP_MISSING");
requireText(chat, "onPaste=", "CHAT_IMAGE_PASTE_MISSING");
requireText(chat, "Vision Model Required", "IMAGE_MODEL_CAPABILITY_GUARD_MISSING");
requireText(chat, "use write_file to save an edited version", "ATTACHMENT_WORK_EDIT_HANDOFF_MISSING");
requireText(chat, "MAX_ATTACHED_AUDIO_BYTES", "AUDIO_ATTACHMENT_SIZE_GUARD_MISSING");
requireText(chat, "encodeAudioBufferAsWav", "AUDIO_WAV_CONVERSION_MISSING");
requireText(chat, "transcribeAudioAttachment", "AUDIO_TRANSCRIPTION_PIPELINE_MISSING");
requireText(chat, "audioTranscriptionChainRef", "AUDIO_TRANSCRIPTION_QUEUE_MISSING");
requireText(chat, "Audio Transcription Failed", "AUDIO_TRANSCRIPTION_ERROR_MISSING");
requireText(chat, "; speech transcript", "AUDIO_TRANSCRIPT_CONTEXT_MISSING");
requireText(chat, ".mp3,.m4a,.aac,.ogg,.webm,.flac", "COMMON_AUDIO_FORMAT_PICKER_MISSING");
requireText(app, "speechSettings={speechSettings}", "SPEECH_SETTINGS_CHAT_HANDOFF_MISSING");
requireText(chat, 'import("mammoth/mammoth.browser")', "DOCX_ATTACHMENT_PARSER_MISSING");
requireText(chat, 'import("pdfjs-dist/legacy/build/pdf.mjs")', "PDF_ATTACHMENT_PARSER_MISSING");
requireText(chat, "extractPdfText", "PDF_TEXT_EXTRACTION_MISSING");
requireText(chat, "extractWordText", "WORD_TEXT_EXTRACTION_MISSING");
requireText(chat, "Scanned PDFs need OCR support", "SCANNED_PDF_GUIDANCE_MISSING");
requireText(chat, ".pdf,.doc,.docx,.pptx,.xlsx", "STANDARD_DOCUMENT_PICKER_MISSING");
requireText(chat, 'import("jszip")', "PPTX_ATTACHMENT_PARSER_MISSING");
requireText(chat, "extractPowerPointText", "PPTX_ATTACHMENT_EXTRACTION_MISSING");
requireText(chat, "sheet.data.map", "XLSX_ATTACHMENT_MULTI_SHEET_MISSING");
requireText(chat, "selectRelevantDocumentChunks", "DOCUMENT_CHUNK_RETRIEVAL_MISSING");
requireText(chat, "MAX_DOCUMENT_CONTEXT_CHARS", "DOCUMENT_CONTEXT_BUDGET_MISSING");
requireText(chat, "selectedPerAttachment", "DOCUMENT_CHUNK_DIVERSITY_MISSING");
requireText(chat, "recentUserContext", "PROJECT_RAG_FOLLOWUP_QUERY_MISSING");
requireText(chat, "retrievedProjectSources", "PROJECT_RAG_SOURCE_METADATA_MISSING");
requireText(chat, 'className="chat-rag-sources"', "PROJECT_RAG_SOURCE_DISCLOSURE_MISSING");
requireText(chat, "setRequestedWorkFile", "PROJECT_RAG_SOURCE_OPEN_MISSING");
requireText(chat, "requestedFile={requestedWorkFile}", "PROJECT_RAG_SOURCE_PANEL_HANDOFF_MISSING");
requireText(workTools, "requestedFile.path", "PROJECT_RAG_SOURCE_FILE_OPEN_MISSING");
requireText(workTools, "setSelectionRange", "PROJECT_RAG_SOURCE_RANGE_SELECTION_MISSING");
requireText(styles, ".chat-rag-sources", "PROJECT_RAG_SOURCE_STYLE_MISSING");
requireText(chat, 'fetch("/api/work/search"', "PROJECT_RAG_CLIENT_MISSING");
requireText(server, 'req.url === "/api/work/search"', "PROJECT_RAG_API_MISSING");
requireText(server, "assertWorkFolderGrant", "PROJECT_RAG_GRANT_GUARD_MISSING");
requireText(projectSearch, "MAX_TOTAL_BYTES", "PROJECT_RAG_INDEX_BUDGET_MISSING");
requireText(projectSearch, "IGNORED_DIRECTORIES", "PROJECT_RAG_DIRECTORY_GUARD_MISSING");
requireText(projectSearch, "entry.isSymbolicLink()", "PROJECT_RAG_SYMLINK_GUARD_MISSING");
requireText(projectSearch, "CACHE_TTL_MS", "PROJECT_RAG_CACHE_MISSING");
requireText(projectSearch, 'new Set([".pdf", ".doc", ".docx", ".pptx", ".xlsx"])', "PROJECT_RAG_DOCUMENT_TYPES_MISSING");
requireText(projectSearch, 'require("mammoth")', "PROJECT_RAG_DOCX_PARSER_MISSING");
requireText(projectSearch, 'import("pdfjs-dist/legacy/build/pdf.mjs")', "PROJECT_RAG_PDF_PARSER_MISSING");
requireText(projectSearch, "extractProjectDocument", "PROJECT_RAG_DOCUMENT_EXTRACTION_MISSING");
requireText(projectSearch, "MAX_DOCUMENT_BYTES", "PROJECT_RAG_DOCUMENT_BUDGET_MISSING");
requireText(projectSearch, 'require("read-excel-file/node")', "PROJECT_RAG_XLSX_PARSER_MISSING");
requireText(projectSearch, "sheet.data.map", "PROJECT_RAG_XLSX_MULTI_SHEET_MISSING");
requireText(projectSearch, 'require("jszip")', "PROJECT_RAG_PPTX_PARSER_MISSING");
requireText(projectSearch, "[Slide ${index + 1}]", "PROJECT_RAG_PPTX_SLIDE_MARKERS_MISSING");
requireText(chat, 'aria-label="Send code to Work Terminal"', "CODE_TO_TERMINAL_ACTION_MISSING");
requireText(chat, "parseWorkActions", "AUTONOMOUS_WORK_ACTION_PARSER_MISSING");
requireText(chat, "executeWorkActions", "AUTONOMOUS_WORK_EXECUTOR_MISSING");
requireText(chat, "MAX_WORK_AGENT_ROUNDS", "AUTONOMOUS_WORK_LOOP_BOUND_MISSING");
requireText(chat, 'tool === "write_file"', "AUTONOMOUS_WORK_WRITE_APPROVAL_MISSING");
requireText(chat, "Automatic Work tool results", "AUTONOMOUS_WORK_FEEDBACK_LOOP_MISSING");
requireText(chat, "Do not ask the user to copy commands", "AUTONOMOUS_WORK_PROMPT_MISSING");
requireText(terminalDock, '"luke:work-terminal-command"', "TERMINAL_CODE_RECEIVER_MISSING");
requireText(workTools, 'fetch("/api/work/file/read"', "WORK_FILE_READER_MISSING");
requireText(workTools, 'fetch("/api/work/directory"', "WORK_DIRECTORY_BROWSER_MISSING");
requireText(workTools, 'className="work-file-breadcrumbs"', "WORK_DIRECTORY_BREADCRUMBS_MISSING");
requireText(workTools, 'aria-label="Filter Work files"', "WORK_FILE_FILTER_MISSING");
requireText(workTools, 'aria-label="Active Work source folder"', "MULTI_SOURCE_WORK_SELECTOR_MISSING");
requireText(workTools, 'fetch("/api/work/file/write"', "WORK_FILE_EDITOR_MISSING");
requireText(workTools, "expectedModifiedAt: openFile.modifiedAt", "WORK_FILE_CONFLICT_TOKEN_MISSING");
requireText(workTools, "This file changed on disk", "WORK_FILE_CONFLICT_UI_MISSING");
requireText(workTools, ">Reload</button>", "WORK_FILE_RELOAD_MISSING");
requireText(workTools, 'event.key.toLowerCase() === "s"', "WORK_FILE_SAVE_SHORTCUT_MISSING");
requireText(server, 'req.url === "/api/work/file/read"', "WORK_FILE_READ_API_MISSING");
requireText(server, 'req.url === "/api/work/directory"', "WORK_DIRECTORY_API_MISSING");
requireText(server, 'req.url === "/api/work/file/write"', "WORK_FILE_WRITE_API_MISSING");
requireText(server, "assertWorkFolderGrant", "WORK_FOLDER_SERVER_GUARD_MISSING");
requireText(folderGrants, "grant.projectId !== owner", "WORK_FOLDER_PROJECT_BINDING_MISSING");
requireText(folderGrants, "grant.root !== canonicalRoot", "WORK_FOLDER_ROOT_BINDING_MISSING");
requireText(workFiles, "approvalGranted !== true", "WORK_FILE_APPROVAL_GUARD_MISSING");
requireText(workFiles, "Work file symlink escaped", "WORK_FILE_SYMLINK_GUARD_MISSING");
requireText(workFiles, "MAX_DOCUMENT_PREVIEW_BYTES", "WORK_DOCUMENT_PREVIEW_BUDGET_MISSING");
requireText(workFiles, "extractProjectDocument", "WORK_DOCUMENT_PREVIEW_PARSER_MISSING");
requireText(workTools, "openFile.readOnly", "WORK_DOCUMENT_READONLY_UI_MISSING");
requireText(actionRunner, 'file === "cat"', "TERMINAL_CAT_MISSING");
requireText(actionRunner, 'file === "head" || file === "tail"', "TERMINAL_HEAD_TAIL_MISSING");
requireText(terminalDock, 'title="Copy Terminal output"', "TERMINAL_COPY_MISSING");
requireText(terminalDock, 'aria-label="Terminal source folder"', "MULTI_SOURCE_TERMINAL_SELECTOR_MISSING");

console.log("PASS: Projects persist locally and link new Chat conversations.");
console.log("PASS: Projects can be created, renamed, pinned, selected and removed.");
console.log("PASS: Native source folders can be added and detached without deleting files.");
console.log("PASS: Project chats are grouped in the sidebar.");
console.log("PASS: Chat and Work modes persist and use distinct model instructions.");
console.log("PASS: Chat and Work mode switching preserves compatible conversations and stable layout.");
console.log("PASS: Work Tools exposes read-only Environment, Review and Files views.");
console.log("PASS: Work Terminal, Browser and external apps are allowlisted and approval-gated.");
console.log("PASS: Composer remains editable during streaming and supports an editable message queue.");
console.log("PASS: Text-only queued messages recover within storage bounds and remain paused until resumed.");
console.log("PASS: Completed Markdown messages reuse rendered output during unrelated Chat updates.");
console.log("PASS: Chat search covers current and project conversations with result navigation.");
console.log("PASS: Edit and retry preserve previous responses as switchable conversation branches.");
console.log("PASS: Work projects expose editable memory and restorable conversation checkpoints.");
console.log("PASS: Composer mentions, slash commands and safe code-to-terminal actions are connected.");
console.log("PASS: Chat accepts dropped, pasted and selected images, text/code, PowerPoint and XLSX attachments.");
console.log("PASS: Common audio attachments are queued, normalized and transcribed before sending.");
console.log("PASS: PDF, DOCX, legacy DOC and common text documents provide readable Chat context.");
console.log("PASS: Large documents and granted Project files use bounded relevant-chunk retrieval.");
console.log("PASS: Work Chat can execute bounded project-scoped tool loops without copy and paste.");
console.log("PASS: Work Files edits confined text files with explicit write approval.");
console.log("PASS: Work Terminal reads files with allowlisted commands and copies output.");
console.log("PASS: Work Terminal remains editable while commands run and processes a removable FIFO queue.");
console.log("PASS: Work Terminal preserves bounded per-project drafts and history with debounced storage.");
console.log("PASS: Frontend Chat Projects foundation validation completed.");
