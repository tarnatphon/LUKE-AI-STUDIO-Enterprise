...
import { Bot, LoaderCircle, Send, Trash2, Square, History, Paperclip, X, ChevronDown, Globe2, Plus, FileCode } from "lucide-react";
import {
  getDownloadProgress,
  getLlmStatus,
  listLlmModels,
                          ...
                          {Array.isArray(displayContent) ? (
                            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                              {displayContent.map((item, idx) => {
                                if (item.type === "text") return <MarkdownRenderer key={idx} content={item.text} onOpenWorkTab={onOpenWorkTab} />;
...
export function MarkdownRenderer({ content, onOpenWorkTab }) {
    ...
    <div className="markdown-body" style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
      {parts.map((part, index) => {
        if (part.startsWith("```") && part.endsWith("```")) {
          ...
          return (
            <div key={index} className="code-block-container" style={{ position: "relative", margin: "6px 0" }}>
              <pre style={{ 
                background: "var(--md-sys-color-surface-variant)", 
              ...
              {onOpenWorkTab && (
                <button
                  ...
                  onClick={() => onOpenWorkTab(code.trim(), lang)}
