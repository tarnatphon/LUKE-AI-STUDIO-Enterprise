...
export default function SocialAgency({ onCreateImage, onCreateVideo, onOpenChat }) {
  ...
  const autoPlan = async (month, postsPerWeek, opts = {}) => {
    const data = await postJson("/api/social-agency/auto-plan", { clientId: activeClient.id, month, postsPerWeek, ...opts });
    return data.preview;
  };

  const applyPlan = withBusy(async (clientId, month, slots, opts = {}) => {
    await postJson("/api/social-agency/auto-plan/apply", { clientId, month, slots, ...opts });
  });
